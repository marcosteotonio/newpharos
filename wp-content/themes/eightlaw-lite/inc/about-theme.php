...
class Eightlaw_Lite_About_Theme {
	public $eightlaw_lite_req_plugins = array(); // For Storing the list of the Required Plugins
	public function __construct() {
		...
		$this->eightlaw_lite_req_plugins = array(

			'instant-demo-importer' => array(
				'slug' => 'instant-demo-importer',
				'name' => __('Instant Demo Importer', 'eightlaw-lite'),
				...
				'info' => __('Instant Demo Importer Plugin adds the feature to Import the Demo Conent with a single click.', 'eightlaw-lite'),
				),
			);

		/** Plugin Installation Ajax **/
		add_action( 'wp_ajax_eightlaw_lite_plugin_installer', array( $this, 'eightlaw_lite_plugin_installer_callback' ) );

		/** Plugin Installation Ajax **/
		add_action( 'wp_ajax_eightlaw_lite_plugin_offline_installer', array( $this, 'eightlaw_lite_plugin_offline_installer_callback' ) );

		/** Plugin Activation Ajax **/
		add_action( 'wp_ajax_eightlaw_lite_plugin_activation', array( $this, 'eightlaw_lite_plugin_activation_callback' ) );

		/** Plugin Activation Ajax (Offline) **/
		add_action( 'wp_ajax_eightlaw_lite_plugin_offline_activation', array( $this, 'eightlaw_lite_plugin_offline_activation_callback' ) );
	}
		...
		<div class="wrap eightlaw-lite-wrap">
			...
			<div class="bottom-block">
				<ul class="eightlaw-lite-tab-wrapper wp-clearfix">
					...
					<li><a href="<?php echo esc_url( admin_url( 'themes.php?page=eightlaw-lite-about&tab=demo_import' ) ); ?>"
						class="eightlaw-lite-tab <?php echo $active_tab == 'demo_import' ? 'eightlaw-lite-tab-active' : ''; ?> "><?php echo esc_html__( 'Import Demo', 'eightlaw-lite' ); ?>
					...
					<li><a href="<?php echo esc_url( admin_url( 'themes.php?page=eightlaw-lite-about&tab=more_wp' ) ); ?>"
						class="eightlaw-lite-tab <?php echo $active_tab == 'more_wp' ? 'eightlaw-lite-tab-active' : ''; ?> "><?php echo esc_html__( 'More WordPress Stuff', 'eightlaw-lite' ); ?>
	...
	public function get_local_dir_path($plugin) {
		...
		$url = wp_nonce_url(admin_url('themes.php??page=eightlaw-lite-about&tab=demo_import'),'bloog-file-installation');
		if (false === ($creds = request_filesystem_credentials($url, '', false, false, null) ) ) {
			...
			/* ========== Plugin Installation Ajax =========== */
			public function eightlaw_lite_plugin_installer_callback(){

				if ( ! current_user_can('install_plugins') )
					wp_die( __( 'Sorry, you are not allowed to install plugins on this site.', 'eightlaw-lite' ) );
				...
				if (! wp_verify_nonce( $nonce, 'eightlaw_lite_plugin_installer_nonce' ))
					wp_die( __( 'Error - unable to verify nonce, please try again.', 'eightlaw-lite') );
				...
				// Get Plugin Info
				$api = $this->eightlaw_lite_call_plugin_api($plugin);
			...
			public function eightlaw_lite_plugin_offline_installer_callback() {
			...
			public function eightlaw_lite_plugin_offline_activation_callback() {
			...
			/** Plugin Activation Ajax **/
			public function eightlaw_lite_plugin_activation_callback(){

				if ( ! current_user_can('install_plugins') )
					wp_die( __( 'Sorry, you are not allowed to activate plugins on this site.', 'eightlaw-lite' ) );

				$nonce = $_POST["nonce"];
				$plugin = $_POST["plugin"];

				// Check our nonce, if they don't match then bounce!
				if (! wp_verify_nonce( $nonce, 'eightlaw_lite_plugin_activate_nonce' ))
					die( __( 'Error - unable to verify nonce, please try again.', 'eightlaw-lite' ) );
				...
				// Get Plugin Info
				$api = $this->eightlaw_lite_call_plugin_api(esc_attr($plugin));
				...
				if($api->name){
					...
					$status = 'failed';
					$msg = esc_html__('There was an error activating $api->name', 'eightlaw-lite');
				}
	...
	if(class_exists('Instant_Demo_Importer')) :
		...
		$demoimporter->demos = array(
			'eightlaw-lite' => array(
				'title' => __('EightLaw Lite Demo', 'eightlaw-lite'),
				'name' => 'eightlaw-lite',
