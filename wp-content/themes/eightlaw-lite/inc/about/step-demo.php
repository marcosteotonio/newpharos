...
<div class="warning-msg"><?php echo __('Please install Recommended Plugins for demo to be imported completely.<br /> And Make sure you download demo in a fresh install. <br /> Your Old Data might be deleted.','eightlaw-lite');?></div>
<?php
wp_enqueue_style( 'plugin-install' );
wp_enqueue_script( 'plugin-install' );
wp_enqueue_script( 'updates' );
$eightlaw_lite_req_plugins = $this->eightlaw_lite_req_plugins;

foreach($eightlaw_lite_req_plugins as $slug=>$plugin) :
	if($plugin['bundled'] == false) {
		?>
		<div class="action-tab warning">
			<h3><?php printf( esc_html__("Install : %s Plugin", 'eightlaw-lite'), $plugin['name'] ); ?></h3>
			<p><?php echo __('Please check the plugins folder inside theme and upload the zip of plugins from plugin uploader.','eightlaw-lite');?></p>
		</div>
		<?php
	} else {
		...
		switch($status['needs']) {
			...
			$label = esc_html__('Install and Activate', 'eightlaw-lite');
			$link = $plugin['location'];
			break;

			case 'deactivate' :
			$btn_class = 'button';
			$label = esc_html__('Deactivate', 'eightlaw-lite');
			$link = admin_url('plugins.php');
			break;

			case 'activate' :
			$btn_class = 'activate-offline button button-primary';
			$label = esc_html__('Activate', 'eightlaw-lite');
			$link = $plugin['location'];
			break;
		}
		?>
